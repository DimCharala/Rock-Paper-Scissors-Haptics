...
class FeatureExtractor():
    def __init__(self, winlen=50, overlap=25):
        self.features = [
            #self.mav,        # Mean Absolute Value
            self.variance,   # Variance
            self.signal_energy,  # Signal Energy
            #self.waveform_length,  # Total Signal Movement
            self.rms         # Root Mean Square
        ]
        if winlen <= overlap:
    ...
    def variance(self, sig):
        """Compute variance for each channel"""
        return np.var(sig, axis=0)[np.newaxis, :]
    
    def signal_energy(self, sig):
        """Compute signal energy for each channel"""
        return np.sum(sig**2, axis=0)[np.newaxis, :]
    
    def waveform_length(self, sig):
        """Compute total signal movement across the window"""
        return np.sum(np.abs(np.diff(sig, axis=0)), axis=0)[np.newaxis, :]
    
    def rms(self, sig):
        """Compute Root Mean Square for each channel"""
        return np.sqrt(np.mean(sig**2, axis=0))[np.newaxis, :]
...
class ClassificationModel:
    ...
    def fit(self, feature_matrix, labels, retained_variance = 0.75, classifier = None):
        ...
        if classifier is None:
            self.classifier = svm.SVC(gamma='scale', C=20)
        else:
        ...
        conf_mat = np.zeros((max(labels)+1, max(labels)+1), dtype=int)
        ...
        for i in range(len(conf_mat)):
            true_positives = conf_mat[i, i]  # Correct predictions for class i
            total_class_i = np.sum(conf_mat[i, :])  # Total instances of class i
            class_accuracy = true_positives / total_class_i if total_class_i > 0 else 0
